            ...
            pub mod run {
                ...
                macro_rules! __export_wasi_cli_run_0_2_4_cabi {
                    ($ty:ident with_types_in $($path_to_types:tt)*) => {
                        const _ : () = { #[export_name = "wasi:cli/run@0.2.4#run"] unsafe
                        extern "C" fn export_run() -> i32 { $($path_to_types)*::
                        _export_run_cabi::<$ty > () } };
                    };
                }
                #[doc(hidden)]
                pub(crate) use __export_wasi_cli_run_0_2_4_cabi;
            }
        }
    }
}
...
#[cfg(target_arch = "wasm32")]
#[link_section = "component-type:wit-bindgen:0.36.0:example:run:example:encoded world"]
#[doc(hidden)]
...
B\x03\x01j\0\0\x01@\0\0\0\x04\0\x03run\x01\x01\x04\0\x12wasi:cli/run@0.2.4\x05\0\
\x04\0\x13example:run/example\x04\0\x0b\x0d\x01\0\x07example\x03\0\0\0G\x09produ\
